...
description: Capture a read-only status snapshot of all infrastructure observed by the SRE agents and post it to chat. Used for the operator-driven `/sre-snapshot` command.
tool: agents.master.tools:capture_status_snapshot
---
# When to use

When the user message is a JSON object with `task: "snapshot"`. The Lambda intake dispatches `/sre-snapshot` slash-command invocations this way. Pass the full JSON payload verbatim as `snapshot_request_json`.
...
- `snapshot_request_json` (required): the user message verbatim. Must be a JSON object with:
  ...
  - `user_id`: who invoked `/sre-snapshot` (logged, not posted).
...
- **Posting**: top-level (not a thread reply) — `/sre-snapshot` is an operational broadcast.
